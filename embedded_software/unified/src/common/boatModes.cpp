...
BoatModeBase* BoatSelfTestMode::execute() {
	...
	this->callCount++;
	//cerr << "Self Test call count: " << callCount << endl;
	_state.servoEnable.set();
	
	// check for errors
	if (_state.health->batteryMon < Conf::get()->startBatMinVolt()) _state.insertFault("Low Battery");
	...
	if (_state.health->batteryMon > Conf::get()->startBatMinVolt()) _state.removeFault("Low Battery");
	if (_state.rc->isValid()) _state.removeFault("RC input invalid");
	
	// Are we done?
	if (_state.recordTime > (start + Conf::get()->selfTestDelay())) {
		if (_state.faultCount()) {
			...
			if ((oldState.getBoatMode() == BoatModeEnum::NAVIGATION) && (_state.getArmState() == ArmButtonStateEnum::ARM)) {
				...
				std::this_thread::sleep_for(Conf::get()->disarmPulseLen());
...
BoatModeBase* BoatDisarmedMode::execute() {
	...
	this->callCount++;
	//cerr << "Disarm call count: " << callCount << endl;
	...
	if (newmode != BoatModeEnum::DISARMED) {
		if (newmode == BoatModeEnum::SELFTEST) {
			...
			std::this_thread::sleep_for(Conf::get()->armPulseLen());
	...
	if (_state.getArmState() == ArmButtonStateEnum::ARM) {
		if (hornOn) {
			if (_state.recordTime > (hornStartTime + Conf::get()->hornTime())) {
...
BoatModeBase* BoatFaultMode::execute() {
	...
	this->callCount++;
	//cerr << "Fault call count: " << callCount << endl;
	_state.throttle->setThrottle(0);
	_state.servoEnable.clear();
...
BoatModeBase* BoatNavigationMode::execute() {
	...
	if (_navMode != _oldNavMode) REMOVE(_oldNavMode);

	// check the battery
	if (_state.health->batteryMon < Conf::get()->lowBatCutoffVolt()) {
